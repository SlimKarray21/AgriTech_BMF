...
import { getProfiles, getSurfaces, getVannes, getPlantes, getSols, getClimats, getRapportsEau, getRapportsSol, deleteSurface, deleteVanne } from "@/services/data-service";
...
import {
  Search, ArrowLeft, User, Users, Mail, Phone, MapPin, ShieldCheck, Calendar,
  CreditCard, Droplets, Leaf, FileText, Eye, Grid3X3, Cpu, CheckCircle, Clock, Plus
} from "lucide-react";
import { useNavigate } from "react-router-dom";
import { toast } from "@/hooks/use-toast";
import { SubscriptionStatus } from "@/pages/shared/basedonnees/SubscriptionStatus";
import { InfoRow } from "@/pages/shared/basedonnees/InfoRow";
import { NewProjectDialog } from "@/pages/shared/basedonnees/NewProjectDialog";
...
export default function BaseDonneesPage() {
  ...
  // Partenaire sélectionné -> ses clients (created_by = id du partenaire).
  const isSelectedPartner = (selectedProfile?.user_role ?? "").toUpperCase() === "PARTENAIRE";
  const partnerClients = useMemo(
    () => (selectedProfile ? allProfiles.filter(c => c.created_by === selectedProfile.id) : []),
  ...
  if (selectedProfile) {
    return (
      <div className="space-y-6">
        ...
        {/* key={selectedUserId} : réinitialise l'onglet actif sur "Informations"
            quand on change d'utilisateur (ex: clic sur un client depuis l'onglet
            Utilisateurs d'un partenaire), sinon l'onglet reste bloqué sur un
            onglet inexistant et le contenu n'apparaît pas. */}
        <Tabs key={selectedUserId} defaultValue="info" className="w-full">
          <TabsList className={`grid w-full ${isSelectedPartner ? "grid-cols-6" : "grid-cols-5"}`}>
            <TabsTrigger value="info" className="gap-1 text-xs"><User className="h-3.5 w-3.5" /> {t("bdd.info")}</TabsTrigger>
            {isSelectedPartner && (
              <TabsTrigger value="utilisateurs" className="gap-1 text-xs"><Users className="h-3.5 w-3.5" /> Utilisateurs</TabsTrigger>
            )}
          ...
          {/* UTILISATEURS (clients du partenaire) */}
          {isSelectedPartner && (
            <TabsContent value="utilisateurs">
              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-2">
                    <Users className="h-4 w-4" /> Clients du partenaire ({partnerClients.length})
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  {partnerClients.length === 0 ? (
                    <p className="text-sm text-muted-foreground">Aucun client rattaché à ce partenaire.</p>
                  ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {partnerClients.map(c => {
                        const cSurfaces = surfaces.filter(s => s.fkUser === c.id).length;
                        const cWater = waterReports.filter((r: any) => r.user_id === c.id).length;
                        const cSoil = soilReports.filter((r: any) => r.user_id === c.id).length;
                        return (
                          <Card
                            key={c.id}
                          ...
                          >
                            <CardContent className="p-4 space-y-2">
                              <div className="flex items-center justify-between">
                                ...
                                <Badge variant="outline" className={`text-[10px] ${roleBadgeClass(c.user_role)}`}>{c.user_role}</Badge>
                              </div>
                              <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
                                <span className="flex items-center gap-1"><Grid3X3 className="h-3 w-3" /> {cSurfaces} parcelle(s)</span>
                                <span className="flex items-center gap-1"><Droplets className="h-3 w-3" /> {cWater} rapport eau</span>
                                <span className="flex items-center gap-1"><Leaf className="h-3 w-3" /> {cSoil} rapport sol</span>
                              </div>
                            </CardContent>
                          </Card>
                        );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredProfiles.map(p => {
          ...
          const isPartner = (p.user_role ?? "").toUpperCase() === "PARTENAIRE";
          const pClients = isPartner ? allProfiles.filter(c => c.created_by === p.id).length : 0;
          ...
          return (
            ...
            >
              <CardContent className="p-4 space-y-3">
                ...
                <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
                  {isPartner && (
                    <span className="flex items-center gap-1 text-blue-600 font-medium"><Users className="h-3 w-3" /> {pClients} client(s)</span>
                  )}
